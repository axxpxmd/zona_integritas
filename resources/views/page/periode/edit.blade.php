...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-xl font-bold text-gray-900">Edit Periode</h2>
            <p class="text-sm text-gray-500 mt-1">Perbarui data periode pengisian kuesioner</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="max-w-3xl">
        <form action="{{ route('periode.update', $periode) }}" method="POST" class="bg-white rounded-xl p-6 space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="copied_from_periode_id"
                        ...
                        <optgroup label="Template">
                            ...
                            <option value="{{ $template->id }}" {{ old('copied_from_periode_id', $periode->copied_from_periode_id) == $template->id ? 'selected' : '' }}>
                        ...
                        <optgroup label="Periode Normal">
                            ...
                            <option value="{{ $p->id }}" {{ old('copied_from_periode_id', $periode->copied_from_periode_id) == $p->id ? 'selected' : '' }}>
                    ...
                    <p class="mt-1.5 text-xs text-gray-500">
                        ...
                        Informasi periode mana yang menjadi sumber copy
                    </p>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="deskripsi"
                          ...
                          class="w-full px-4 py-2.5 bg-white border @error('deskripsi') border-red-300 @else border-gray-300 @enderror rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all resize-none">{{ old('deskripsi', $periode->deskripsi) }}</textarea>
                @error('deskripsi')
                <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                <label class="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox"
                           ...
                           id="is_template"
                           value="1"
                           {{ old('is_template', $periode->is_template) ? 'checked' : '' }}
