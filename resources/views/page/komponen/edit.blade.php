...
<div class="max-w-3xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Perbarui data komponen Zona Integritas</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl">
        <form action="{{ route('komponen.update', $komponen) }}" method="POST" class="p-4 sm:p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number"
                           name="bobot"
                           id="bobot"
                           value="{{ old('bobot', $komponen->bobot) }}"
                           placeholder="0-100"
                           step="0.01"
                           min="0"
                           max="100"
                ...
                <div>
                    ...
                    <input type="number"
                           name="urutan"
                           id="urutan"
                           value="{{ old('urutan', $komponen->urutan) }}"
                           placeholder="1, 2, 3, ..."
                           min="0"
            ...
            <div>
                ...
                <textarea name="deskripsi"
                          ...
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors resize-none @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $komponen->deskripsi) }}</textarea>
                @error('deskripsi')
                <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status"
                    ...
                    <option value="1" {{ old('status', $komponen->status) == '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('status', $komponen->status) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                @error('status')
                <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                @enderror
