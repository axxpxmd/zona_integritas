...
@section('page-title', 'Data Pertanyaan')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Data Pertanyaan</h2>
            <p class="text-sm text-gray-600 mt-1">Kelola data pertanyaan kuesioner</p>
        </div>
    ...
    {{-- Filter Card --}}
    <div class="bg-white rounded-xl p-5">
        <form method="GET" action="{{ route('pertanyaan.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <div class="lg:col-span-3">
                    <input type="text"
                           name="search"
                           value="{{ request('search') }}"
                           placeholder="Cari pertanyaan, kode, penjelasan, atau indikator..."
                ...
                <select name="tipe_jawaban"
                        class="px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
                    <option value="">Semua Tipe Jawaban</option>
                    <option value="ya_tidak" {{ request('tipe_jawaban') == 'ya_tidak' ? 'selected' : '' }}>Ya/Tidak</option>
                    <option value="pilihan_ganda" {{ request('tipe_jawaban') == 'pilihan_ganda' ? 'selected' : '' }}>Pilihan Ganda</option>
                    <option value="angka" {{ request('tipe_jawaban') == 'angka' ? 'selected' : '' }}>Angka</option>
                    <option value="teks" {{ request('tipe_jawaban') == 'teks' ? 'selected' : '' }}>Teks</option>
                </select>
                <select name="status"
                        class="px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @if(request()->hasAny(['search', 'komponen_id', 'kategori_id', 'sub_kategori_id', 'indikator_id', 'tipe_jawaban', 'status']))
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Pertanyaan</th>
                        <th class="px-4 py-3.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Indikator</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-4">
                            <p class="font-medium text-gray-900 text-sm line-clamp-2">{{ $pertanyaan->pertanyaan }}</p>
                            @if($pertanyaan->penjelasan)
                            <p class="text-xs text-gray-500 mt-1 line-clamp-1">{{ $pertanyaan->penjelasan }}</p>
                            @endif
                        </td>
                        <td class="px-4 py-4">
                            <p class="text-sm font-medium text-gray-900">{{ $pertanyaan->indikator->nama }}</p>
                            <p class="text-xs text-gray-500 mt-0.5">{{ $pertanyaan->indikator->subKategori->nama }}</p>
                        </td>
                        ...
                        <td class="px-4 py-4 text-center">
                            <span class="text-sm text-gray-700">{{ $pertanyaan->urutan }}</span>
                        </td>
                        <td class="px-4 py-4 text-center">
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('pertanyaan.show', $pertanyaan) }}"
                                ...
                                <form action="{{ route('pertanyaan.destroy', $pertanyaan) }}"
                                      method="POST"
                    ...
                    <tr>
                        <td colspan="7" class="px-5 py-12 text-center">
                            ...
                            <p class="text-gray-500 font-medium">Tidak ada data pertanyaan</p>
                            <p class="text-sm text-gray-400 mt-1">Silakan tambah data pertanyaan baru</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="grid grid-cols-1 gap-4 lg:hidden">
        ...
        <div class="bg-white rounded-xl p-4 space-y-3">
            <div class="flex items-start justify-between gap-3">
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        @if($pertanyaan->tipe_jawaban === 'ya_tidak')
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Ya/Tidak</span>
                        @elseif($pertanyaan->tipe_jawaban === 'pilihan_ganda')
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-700">Pilihan Ganda</span>
                        @elseif($pertanyaan->tipe_jawaban === 'angka')
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700">Angka</span>
                        @else
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">Teks</span>
                        @endif
                    </div>
                    <h3 class="font-semibold text-gray-900 line-clamp-2">{{ $pertanyaan->pertanyaan }}</h3>
                    <p class="text-xs text-gray-500 mt-1">{{ $pertanyaan->indikator->nama }}</p>
                </div>
                @if($pertanyaan->status == 1)
            ...
            @if($pertanyaan->penjelasan)
            <p class="text-sm text-gray-600 line-clamp-2">{{ $pertanyaan->penjelasan }}</p>
            @endif

            <div class="flex items-center gap-2 pt-2 border-t border-gray-100">
                <span class="text-xs text-gray-600">Urutan: <strong>{{ $pertanyaan->urutan }}</strong></span>
            </div>
...
@push('scripts')
<script>
// 4-level cascading filter: Komponen -> Kategori -> Sub Kategori -> Indikator
...
komponenFilter?.addEventListener('change', function() {
    ...
    filterOptions(subKategoriFilter, komponenId, 'data-komponen');
    filterOptions(indikatorFilter, komponenId, 'data-komponen');
    kategoriFilter.value = '';
    subKategoriFilter.value = '';
    indikatorFilter.value = '';
});
...
kategoriFilter?.addEventListener('change', function() {
    ...
    subKatOptions.forEach(option => {
        ...
        let show = true;
        if (kategoriId !== '' && optionKategoriId !== kategoriId) show = false;
        if (komponenId !== '' && optionKomponenId !== komponenId) show = false;

        option.style.display = show ? 'block' : 'none';
    });

    // Filter indikator juga
...
// Filter indikator berdasarkan sub kategori
subKategoriFilter?.addEventListener('change', function() {
    ...
    indOptions.forEach(option => {
        ...
        const optionSubKatId = option.getAttribute('data-subkategori');
        const optionKategoriId = option.getAttribute('data-kategori');
        const optionKomponenId = option.getAttribute('data-komponen');

        let show = true;
        if (subKategoriId !== '' && optionSubKatId !== subKategoriId) show = false;
        if (kategoriId !== '' && optionKategoriId !== kategoriId) show = false;
        if (komponenId !== '' && optionKomponenId !== komponenId) show = false;
