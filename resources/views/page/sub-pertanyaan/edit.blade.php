...
@section('title', 'Edit Sub Pertanyaan')
@section('page-title', 'Edit Sub Pertanyaan')
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4 bg-white px-6 py-4 rounded-xl">
        ...
        <div class="flex items-center gap-3 flex-1">
            ...
            <div>
                <h1 class="text-xl font-bold text-gray-900">Edit Sub Pertanyaan</h1>
                <p class="text-sm text-gray-600">Perbarui data sub pertanyaan</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('sub-pertanyaan.update', $subPertanyaan) }}" method="POST" class="bg-white rounded-xl p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="pertanyaan_id" id="pertanyaan_id" required
                    ...
                    @foreach($pertanyaans as $pertanyaan)
                        <option value="{{ $pertanyaan->id }}" {{ old('pertanyaan_id', $subPertanyaan->pertanyaan_id) == $pertanyaan->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="kode" id="kode" required maxlength="20"
                           value="{{ old('kode', $subPertanyaan->kode) }}"
                ...
                <div>
                    ...
                    <select name="tipe_input" id="tipe_input" required
                        ...
                        <option value="jumlah" {{ old('tipe_input', $subPertanyaan->tipe_input) == 'jumlah' ? 'selected' : '' }}>Jumlah</option>
                        <option value="persen" {{ old('tipe_input', $subPertanyaan->tipe_input) == 'persen' ? 'selected' : '' }}>Persen (%)</option>
                        <option value="angka" {{ old('tipe_input', $subPertanyaan->tipe_input) == 'angka' ? 'selected' : '' }}>Angka</option>
                        <option value="teks" {{ old('tipe_input', $subPertanyaan->tipe_input) == 'teks' ? 'selected' : '' }}>Teks</option>
                    </select>
                    @error('tipe_input')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea name="pertanyaan" id="pertanyaan" rows="3" required
                          ...
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none @error('pertanyaan') border-red-300 @enderror">{{ old('pertanyaan', $subPertanyaan->pertanyaan) }}</textarea>
                @error('pertanyaan')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div id="satuan_field" style="display: none;">
                    ...
                    <input type="text" name="satuan" id="satuan" maxlength="50"
                           value="{{ old('satuan', $subPertanyaan->satuan) }}"
                ...
                <div>
                    ...
                    <input type="number" name="urutan" id="urutan"
                           value="{{ old('urutan', $subPertanyaan->urutan) }}"
                           min="0"
            ...
            <div id="formula_field" style="display: none;">
                ...
                <textarea name="formula" id="formula" rows="2"
                          ...
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none @error('formula') border-red-300 @enderror">{{ old('formula', $subPertanyaan->formula) }}</textarea>
                @error('formula')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="penjelasan" id="penjelasan" rows="3"
                          ...
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none @error('penjelasan') border-red-300 @enderror">{{ old('penjelasan', $subPertanyaan->penjelasan) }}</textarea>
                @error('penjelasan')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="1" {{ old('status', $subPertanyaan->status) == '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('status', $subPertanyaan->status) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                @error('status')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
