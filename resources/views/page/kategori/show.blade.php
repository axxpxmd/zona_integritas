...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
            <h2 class="text-2xl font-bold text-gray-900">Detail Kategori</h2>
    ...
    <div class="bg-white rounded-xl p-6">
        <div class="flex items-start gap-4">
            <div class="w-16 h-16 bg-primary/10 rounded-xl flex items-center justify-center flex-shrink-0">
                <span class="text-2xl font-bold text-primary">{{ $kategori->kode }}</span>
            </div>
            <div class="flex-1 min-w-0">
                <h3 class="text-2xl font-bold text-gray-900">{{ $kategori->nama }}</h3>
                <div class="flex items-center gap-3 mt-2">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-sm font-medium bg-purple-100 text-purple-700">
                        ...
                        {{ $kategori->komponen->kode }} - {{ $kategori->komponen->nama }}
                    </span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
            <div class="bg-blue-50 rounded-xl p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-blue-700">{{ number_format($kategori->bobot, 2) }}%</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-purple-50 rounded-xl p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-purple-700">{{ $kategori->urutan }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-green-50 rounded-xl p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xs text-green-600 font-medium">Jumlah Sub Kategori</p>
                        <p class="text-xl font-bold text-green-700">{{ $kategori->subKategoris->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 pt-6 border-t border-gray-200">
            ...
            <p class="text-gray-600 leading-relaxed">{{ $kategori->deskripsi }}</p>
        </div>
        @endif
        ...
        <div class="mt-6 pt-6 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600">
            ...
            <div class="flex items-center gap-2">
                ...
                <span>Diperbarui: {{ $kategori->updated_at->format('d M Y H:i') }}</span>
            </div>
        </div>
    </div>

    {{-- Sub Kategoris Section --}}
    @if($kategori->subKategoris->count() > 0)
    <div class="bg-white rounded-xl p-6">
        <h3 class="text-lg font-bold text-gray-900 mb-4">Sub Kategori Terkait</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @foreach($kategori->subKategoris as $subKat)
            <div class="border border-gray-200 rounded-lg p-4 hover:border-primary/30 hover:bg-primary/5 transition-all">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex-1">
                        ...
                        <h4 class="font-semibold text-gray-900 text-sm">{{ $subKat->nama }}</h4>
                    </div>
                </div>
                <div class="flex items-center gap-4 mt-3 text-xs text-gray-600">
                    <div class="flex items-center gap-1">
                        ...
                        <span>Bobot: {{ number_format($subKat->bobot, 2) }}%</span>
                    </div>
                    <div class="flex items-center gap-1">
                        <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/>
                        </svg>
                        <span>Urutan: {{ $subKat->urutan }}</span>
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <div class="bg-white rounded-xl p-8 text-center">
        ...
        <p class="text-gray-500 font-medium">Belum ada sub kategori</p>
        <p class="text-sm text-gray-400 mt-1">Sub kategori akan ditampilkan di sini</p>
    </div>
    @endif
</div>
@endsection
