    ...
    <div class="space-y-6">
        ...
        @if ($activePeriode)
            ...
            @if (auth()->user()->role === 'operator' && !empty($operatorStats))
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white rounded-xl p-5 col-span-2 md:col-span-2">
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Progress Pengisian
                                    LKE</p>
                                <div class="flex items-end gap-2 mt-1">
                                    <span class="text-3xl font-bold text-gray-900">{{ $s['persenPengisian'] }}%</span>
                                    <span class="text-sm text-gray-500 mb-0.5">{{ $s['totalDiisi'] }} /
                                        {{ $s['totalRequired'] }}
                                        jawaban</span>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center justify-between mt-2">
                            <span class="text-xs text-gray-500">Belum diisi:
                                {{ $s['totalRequired'] - $s['totalDiisi'] }}</span>
                            @if ($s['isKirimFinal'])
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="bg-white rounded-xl p-5 flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 flex-shrink-0">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            @if ($s['tanggalMulaiRevisi'])
                                <div class="flex items-center justify-between">
                                    ...
                                    <span
                                        class="font-medium text-gray-800">{{ $s['tanggalMulaiRevisi']->format('d M Y') }}
                                        —
            ...
            @if (auth()->user()->role === 'admin')
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white rounded-xl p-5">
                        ...
                        <p class="text-3xl font-bold text-green-600">{{ $opdCompleted }}</p>
                    </div>
                    <div class="bg-white rounded-xl p-5">
                        <div class="flex items-center justify-between mb-3">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Dalam Proses</p>
                        ...
                        <p class="text-3xl font-bold text-yellow-600">{{ $opdInProgress }}</p>
                    </div>
                    <div class="bg-white rounded-xl p-5">
                        <div class="flex items-center justify-between mb-3">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Belum Mengisi</p>
            ...
            @if (in_array(auth()->user()->role, ['verifikator', 'admin']) && !empty($verifikatorStats))
                        ...
                        <div>
                            ...
                            <p class="text-xs {{ $v['isVerifActive'] ? 'text-teal-700' : 'text-gray-500' }} mt-0.5">
                                @if ($v['startVerif'] && $v['endVerif'])
                                    Jadwal verifikasi: {{ $v['startVerif']->format('d M Y') }} s/d
                                    {{ $v['endVerif']->format('d M Y') }}
                                @else
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white rounded-xl p-5">
                        ...
                        <p
                            class="text-3xl font-bold {{ $v['totalMenungguDicekUlang'] > 0 ? 'text-orange-600' : 'text-gray-300' }}">
                                    ...
                                    @foreach ($v['opdProgressVerif'] as $row)
                                        <tr class="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td class="px-5 py-3 text-center">
                                                <div
                                                    class="inline-flex flex-col items-center justify-center bg-gray-50 border border-gray-200 rounded-lg p-2 min-w-[7rem]">
                                                    @if ($row->isFinal)
                                                        <span
                                                    ...
                                                    <div class="flex items-center gap-1">
                                                        <span
                                                            class="text-sm font-bold {{ $row->totalDiisi == $row->totalRequired && $row->totalRequired > 0 ? 'text-green-600' : 'text-gray-800' }}">
                                                            {{ $row->totalDiisi }}
                                                        </span>
                                                        <span class="text-xs text-gray-500 font-medium">/
                                                            {{ $row->totalRequired }}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-5 py-3">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        class="text-xs font-semibold text-gray-600">{{ $row->persen }}%</span>
                                                </div>
                                            </td>
                                            <td class="px-5 py-3">
                                                @if ($row->isFinal)
            ...
            @if (in_array(auth()->user()->role, ['verifikator_menpan', 'admin']) && !empty($menpanStats))
                        ...
                        <div>
                            ...
                            <p class="text-xs {{ $m['isVerifActive'] ? 'text-teal-700' : 'text-gray-500' }} mt-0.5">
                                @if ($m['startVerif'] && $m['endVerif'])
                                    Jadwal verifikasi: {{ $m['startVerif']->format('d M Y') }} s/d
                                    {{ $m['endVerif']->format('d M Y') }}
                                @else
                                    Jadwal verifikasi belum ditentukan
                                @endif
                            </p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    ...
                    <div class="bg-white rounded-xl p-5 col-span-1 md:col-span-1">
                        <div class="flex items-center justify-between mb-3">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Progress Verifikasi
                                Menpan</p>
                            <div class="w-8 h-8 rounded-lg bg-teal-50 flex items-center justify-center text-teal-600">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="bg-white rounded-xl p-5">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Status Pengecekan
                            Menpan</p>
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-1.5">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach ($m['opdProgressMenpan'] as $row)
                                        <tr class="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td class="px-5 py-3 text-center"><span
                                                    class="font-semibold text-green-700">{{ $row->disetujui }}</span>
                                            </td>
                                            <td class="px-5 py-3 text-center"><span
                                                    class="font-semibold text-gray-500">{{ $row->belum }}</span></td>
                                            <td class="px-5 py-3">
        ...
        @else
            <div class="bg-yellow-50 text-yellow-800 rounded-xl p-6 border border-yellow-200">
                ...
                <p class="mt-2">Belum ada periode evaluasi yang aktif. Silahkan atur periode aktif terlebih dahulu di
                    menu
                    manajemen periode.</p>
            </div>
        @endif
