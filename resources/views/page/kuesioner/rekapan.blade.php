...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1 flex items-center flex-wrap gap-2">
            <span>Periode:</span>
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-primary border border-blue-100">
    ...
    <div class="flex gap-2 self-start sm:self-auto">
        ...
        <a href="{{ route('kuesioner.show', $periode->id) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-semibold text-gray-600 hover:text-gray-900 hover:bg-gray-50 shadow-sm hover:shadow transition-all duration-200">
...
<div class="mb-8 border-b border-gray-200 flex flex-wrap gap-6">
    <button type="button" onclick="switchTab('operator')" id="tab-operator" class="pb-3 text-sm font-semibold transition-all duration-200 flex items-center gap-2 border-b-2 border-primary text-primary">
    ...
    <button type="button" onclick="switchTab('verifikator')" id="tab-verifikator" class="pb-3 text-sm font-medium transition-all duration-200 flex items-center gap-2 border-b-2 border-transparent text-gray-500 hover:text-gray-800 hover:border-gray-300">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
        Jawaban Verifikator
    </button>
    <button type="button" onclick="switchTab('menpan')" id="tab-menpan" class="pb-3 text-sm font-medium transition-all duration-200 flex items-center gap-2 border-b-2 border-transparent text-gray-500 hover:text-gray-800 hover:border-gray-300">
...
<div id="content-{{ $role }}" class="{{ $role == 'operator' ? 'block' : 'hidden' }} space-y-6">
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-8 text-center mt-4">
        ...
        <p class="text-gray-600 max-w-lg mx-auto">
            Proses pengisian Lembar Kerja Evaluasi untuk peran <strong>{{ strtoupper($role) }}</strong> belum divalidasi/diselesaikan seluruhnya. Rekapan hanya akan tampil ketika semua pertanyaan sudah selesai diverifikasi atau dikirim.
        </p>
    </div>
    @else
    <!-- Summary Cards -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <!-- Pengungkit Card -->
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200/50 hover:shadow-md hover:border-gray-200 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-gray-400">Total Pengungkit (A)</span>
                    <div class="flex items-baseline gap-1.5 mt-1.5">
                        <span class="text-3xl font-extrabold text-gray-900 tracking-tight">{{ number_format($totalPengungkitNilai, 2) }}</span>
                        <span class="text-xs text-gray-400 font-medium">/ {{ number_format($totalPengungkitBobot, 2) }}</span>
                    </div>
                </div>
            ...
            <div class="mt-6">
                ...
                <div class="w-full bg-gray-100 rounded-full h-2">
                    <div class="bg-primary h-2 rounded-full transition-all duration-500" style="width: {{ $pengungkitPersen }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200/50 hover:shadow-md hover:border-gray-200 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-gray-400">Total Hasil (B)</span>
                    <div class="flex items-baseline gap-1.5 mt-1.5">
                        <span class="text-3xl font-extrabold text-gray-900 tracking-tight">{{ number_format($totalHasilNilai, 2) }}</span>
                        <span class="text-xs text-gray-400 font-medium">/ {{ number_format($totalHasilBobot, 2) }}</span>
                    </div>
                </div>
                <div class="p-3 bg-emerald-50 text-emerald-600 rounded-xl border border-emerald-100/50 shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <div class="mt-6">
                ...
                <div class="w-full bg-gray-100 rounded-full h-2">
                    <div class="bg-emerald-500 h-2 rounded-full transition-all duration-500" style="width: {{ $hasilPersen }}%"></div>
                </div>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-sm text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="bg-slate-50">
                        <td colspan="6" class="px-6 py-3.5 font-bold text-gray-800">
                            <div class="flex items-center gap-2.5">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                                </svg>
                                <span class="tracking-wide text-medium font-bold uppercase text-gray-700">A. Pengungkit</span>
                            </div>
                        </td>
                    </tr>
                    ...
                    @foreach($rekapPengungkit as $area)
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                            <td class="px-6 py-4 border-r border-gray-100 text-gray-700 font-medium">
                                <span class="text-gray-400 font-normal mr-1">{{ $loop->iteration }}.</span> {{ $area['nama'] }}
                            </td>
                            <td class="px-6 py-4 border-r border-gray-100 text-center font-semibold text-gray-600">
                                {{ number_format($bobotArea, 2) }}
                            </td>
                            <td class="px-6 py-4 border-r border-gray-100 text-center text-primary font-medium">
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @endphp
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $badgeColor }} shadow-sm">
                    ...
                    <tr class="bg-slate-50/50 font-bold text-gray-900 border-t border-b border-gray-200">
                        <td class="px-6 py-4 border-r border-gray-200 text-right uppercase tracking-wider text-xs text-gray-500 font-semibold">Total Pengungkit (A)</td>
                        <td class="px-6 py-4 border-r border-gray-200 text-center text-gray-800">{{ number_format($totalPengungkitBobot, 2) }}</td>
                        <td colspan="2" class="px-6 py-4 border-r border-gray-200 bg-slate-50/20 text-center text-gray-400 italic text-xs font-normal">-</td>
                        <td class="px-6 py-4 border-r border-gray-200 text-center text-primary">{{ number_format($totalPengungkitNilai, 2) }}</td>
                        <td class="px-6 py-4 text-center">
                            @php
                                $badgeColor = 'bg-red-50 text-red-700 border-red-100';
                            ...
                            @endphp
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border {{ $badgeColor }} shadow-sm">
                                <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                    ...
                    <tr class="bg-slate-50 border-l-4 border-emerald-500 border-t border-gray-200">
                        <td colspan="6" class="px-6 py-3.5 font-bold text-gray-800">
                            <div class="flex items-center gap-2.5">
                                ...
                                <span class="tracking-wide text-medium font-bold uppercase text-gray-700">B. Hasil</span>
                            </div>
                        </td>
                    </tr>
                    ...
                    @foreach($rekapHasil as $hasil)
                        ...
                        @endphp
                        <tr class="hover:bg-slate-50/50 transition-colors duration-150 bg-slate-50/10 font-bold border-t border-gray-200">
                            <td class="px-6 py-4 border-r border-gray-100 text-gray-800 font-bold">
                                <span class="text-gray-400 font-semibold mr-1">{{ $hasil['kode'] ?? $loop->iteration }}.</span> {{ $hasil['nama'] }}
                            </td>
                            <td class="px-6 py-4 border-r border-gray-100 text-center text-gray-800 font-extrabold">
                                {{ number_format($hasil['bobot'], 2) }}
                            </td>
                            <td colspan="2" class="px-6 py-4 border-r border-gray-100 text-center text-gray-400 italic text-xs font-normal bg-slate-50/20">
                                -
                            </td>
                            <td class="px-6 py-4 border-r border-gray-100 text-center text-gray-800 font-extrabold">
                        ...
                        @foreach($hasil['subs'] as $sub)
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                                <td class="px-6 py-4 border-r border-gray-100 text-gray-700 pl-12 font-medium">
                                    <span class="text-gray-400 font-normal mr-1">{{ $sub['kode'] }}.</span> {{ $sub['nama'] }}
                                </td>
                                <td class="px-6 py-4 border-r border-gray-100 text-center font-semibold text-gray-600">
                                    {{ number_format($sub['bobot'], 2) }}
                                </td>
                                <td colspan="2" class="bg-slate-50/10 px-6 py-4 border-r border-gray-100 text-center text-gray-400 italic text-xs font-normal">
                                    -
                                </td>
                                <td class="px-6 py-4 border-r border-gray-100 text-center font-bold text-gray-800">
                    ...
                    {{-- Subtotal Hasil --}}
                    <tr class="bg-slate-50/50 font-bold text-gray-900 border-t border-b border-gray-200">
                        <td class="px-6 py-4 border-r border-gray-200 text-right uppercase tracking-wider text-xs text-gray-500 font-semibold">Total Hasil (B)</td>
                        <td class="px-6 py-4 border-r border-gray-200 text-center text-gray-800">{{ number_format($totalHasilBobot, 2) }}</td>
                        <td colspan="2" class="px-6 py-4 border-r border-gray-200 bg-slate-50/20 text-center text-gray-400 italic text-xs font-normal">-</td>
                        <td class="px-6 py-4 border-r border-gray-200 text-center text-emerald-600">{{ number_format($totalHasilNilai, 2) }}</td>
                        <td class="px-6 py-4 text-center">
                            @php
                                $badgeColor = 'bg-red-50 text-red-700 border-red-100';
                                if ($hasilPersen >= 90) {
...
<script>
    function switchTab(role) {
        ...
        roles.forEach(r => {
            ...
            tab.classList.add('border-transparent', 'text-gray-500', 'font-medium', 'hover:text-gray-800', 'hover:border-gray-300');
        });
