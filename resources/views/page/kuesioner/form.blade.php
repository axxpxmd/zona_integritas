...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl overflow-hidden">
        ...
        {{-- Indikator Content --}}
        <form action="{{ route('kuesioner.submit') }}" method="POST" enctype="multipart/form-data" id="kuesionerForm">
            @csrf
                        ...
                        @php
                            ...
                            if (!$jawabanItem) {
                                foreach ($pertanyaan->subPertanyaans as $sp) {
                                    ...
                                    if (isset($jawabans[$spKey])) {
                                        $jawabanItem = $jawabans[$spKey];
                                        break;
                                    }
                                }
                            }
                                        ...
                                        <ul class="space-y-1 mt-1 text-[13px] ml-1">
                                            <li class="text-gray-600 flex items-center justify-between gap-3 p-1.5 hover:bg-gray-50 rounded-lg transition-colors group">
                                                <div class="flex items-center gap-2 min-w-0">
                                                    ...
                                                    <a href="{{ route('kuesioner.file.view', $jawabanItem->id) }}" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline font-medium truncate">
            ...
            <div class="mt-6 pt-6 border-t border-gray-200">
                <button type="submit" id="btnSubmitKuesioner" class="w-full inline-flex items-center justify-center gap-2 px-6 py-3 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors font-medium disabled:opacity-70 disabled:cursor-not-allowed disabled:bg-gray-400" @if($isLocked) disabled @endif>
                    ...
                    <svg id="submitSpinner" class="hidden w-5 h-5 animate-spin text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span id="submitText">Simpan Jawaban</span>
                </button>
            </div>
            </div>
            </fieldset>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Hide the original save icon and show the loading spinner
        if (icon) icon.classList.add('hidden');
        if (spinner) spinner.classList.remove('hidden');

        // Update the text
        if (text) text.textContent = 'Menyimpan Jawaban...';
    });
    ...
    window.deleteFile = function(fileId, element) {
        ...
        .then(result => {
            if (result.success) {
                ...
                li.remove();
                // Check if list is empty, show "Belum ada dokumen" if so
                if (list && list.children.length === 0) {
                    const container = list.parentElement;
                    list.remove();
                    container.insertAdjacentHTML('afterbegin', '<div class="text-[12px] text-gray-500 italic mt-1">Belum ada dokumen yang diupload.</div>');
                }
            } else {
    ...
    // Function to delete legacy file via AJAX
    window.deleteLegacyFile = function(jawabanId, element) {
        if (!confirm('Apakah Anda yakin ingin menghapus file ini?')) return;
        ...
        fetch('{{ route("kuesioner.file.delete.legacy", ":id") }}'.replace(':id', jawabanId), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        .then(result => {
            if (result.success) {
                const container = li.closest('.space-y-3');
                li.remove();
                if (container) {
                    container.insertAdjacentHTML('afterbegin', '<div class="text-[12px] text-gray-500 italic mt-1">Belum ada dokumen yang diupload.</div>');
                }
