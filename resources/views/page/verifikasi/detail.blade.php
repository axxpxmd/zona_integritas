...
@section('content')
    <div class="space-y-6">
        ...
        @if (session('success'))
            <div
                class="bg-green-100 border border-green-200 text-green-800 rounded-lg px-4 py-3 mb-4 flex items-center gap-2">
        ...
        @if (!$isCanVerify)
            <div
                class="bg-amber-50 border border-amber-300 text-amber-800 rounded-lg px-4 py-3 mb-4 flex items-center gap-3">
                ...
                <div class="text-sm">
                    ...
                    <p>Anda hanya dapat melihat data.
                        @if ($startVerif && $endVerif)
                            Waktu verifikasi: <span class="font-semibold">{{ $startVerif->format('d M Y') }} s/d
                                {{ $endVerif->format('d M Y') }}</span>.
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2">
                        ...
                        @if ($startVerif && $endVerif)
                            @if (\Carbon\Carbon::now()->lt($startVerif))
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-200">Belum
                                    Dimulai</span>
                            @elseif(\Carbon\Carbon::now()->gt($endVerif))
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 border border-red-200">Waktu
                                    Habis</span>
                            @else
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-200">Aktif</span>
                            @endif
                        @endif
                    </h2>
                    <p class="text-sm text-gray-500 mt-0.5">
                        ...
                        @if ($startVerif && $endVerif)
                            • <span class="font-medium text-gray-700">Waktu Verifikasi: {{ $startVerif->format('d M Y') }}
                                s/d {{ $endVerif->format('d M Y') }}</span>
                        @endif
                    </p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg p-4">
            <div class="flex flex-wrap items-center gap-2 text-sm">
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-100 text-purple-700 font-medium">
        ...
        <div class="bg-white rounded-xl overflow-hidden">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <div class="text-sm text-gray-600 mb-1">
                            <span class="font-semibold text-gray-900">Indikator {{ $currentPage }}</span> dari
                            {{ $totalIndikator }}
                        </div>
                        <div class="text-xs text-gray-500">{{ $currentIndikator->kode }}. {{ $currentIndikator->nama }}
                        </div>
                    </div>
            ...
            {{-- Indikator Content --}}
            <form action="{{ route('verifikasi.store', [$periode->id, $opd->id, $subKategori->id]) }}" method="POST"
                id="verifikasiForm">
                @csrf
                ...
                <div class="p-6">
                    <div class="mb-6">
                        ...
                        <div class="flex items-start gap-2 mb-4">
                            <span
                                class="inline-flex items-center justify-center w-6 h-6 bg-[#0E7C7B]/10 text-[#0E7C7B] rounded text-sm font-bold flex-shrink-0 mt-0.5">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-0.5 bg-accent/20 text-gray-700 rounded text-xs font-medium">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-4">
                                    <div class="text-center">
                                        ...
                                        <p class="text-lg font-bold text-gray-900">
                                            {{ $nilaiIndikator['pertanyaan_terverifikasi'] }}/{{ $nilaiIndikator['total_pertanyaan'] }}
                                        </p>
                                    </div>
                                    <div class="h-10 w-px bg-gray-300"></div>
                                    <div class="text-center">
                                        <p class="text-xs text-gray-500 mb-1">Rata-rata Nilai</p>
                                        <p class="text-lg font-bold text-[#0E7C7B]">
                                            {{ number_format($nilaiIndikator['rata_rata_nilai'], 2) }}</p>
                                    </div>
                                    <div class="h-10 w-px bg-gray-300"></div>
                                    <div class="text-center">
                                        <p class="text-xs text-gray-500 mb-1">Nilai Indikator</p>
                                        <p class="text-lg font-bold text-green-600">
                                            {{ number_format($nilaiIndikator['nilai_indikator'], 2) }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="text-sm font-bold {{ $nilaiIndikator['persen_capaian'] >= 80 ? 'text-green-600' : ($nilaiIndikator['persen_capaian'] >= 50 ? 'text-yellow-600' : 'text-red-600') }}">
                        ...
                        <div class="space-y-4">
                            @foreach ($currentIndikator->pertanyaans as $pertanyaanIndex => $pertanyaan)
                                @php
                                    ...
                                    $nilaiTampil =
                                        $pertanyaanNilai && $pertanyaanNilai['nilai'] !== null
                                            ? $pertanyaanNilai['nilai']
                                            : null;
                                    $statusVerifikasi = $jawabanParent
                                        ? $jawabanParent->status_verifikasi
                                        : 'belum_diverifikasi';
                                    $menungguDicekUlang = $jawabanParent
                                        ? (bool) $jawabanParent->menunggu_dicek_ulang
                                        : false;
                                    ...
                                    if ($pertanyaan->has_sub_pertanyaan) {
                                        foreach ($pertanyaan->subPertanyaans as $sp) {
                                            ...
                                            if (isset($jawabanMap[$spKey])) {
                                                ...
                                                $verifikatorSubValues[$sp->id] =
                                                    $jawabanSub->verifikator_jawaban_angka ??
                                                    $jawabanSub->jawaban_angka;
                                                if (
                                                    $jawabanSub->verifikator_jawaban_angka !== null &&
                                                    $jawabanSub->verifikator_jawaban_angka != $jawabanSub->jawaban_angka
                                                ) {
                                    ...
                                    if (!$pertanyaan->has_sub_pertanyaan && $jawabanParent) {
                                        if ($pertanyaan->tipe_jawaban === 'angka') {
                                            $showOperatorAnswer =
                                                $jawabanParent->verifikator_jawaban_angka !== null &&
                                                $jawabanParent->verifikator_jawaban_angka !=
                                                    $jawabanParent->jawaban_angka;
                                        } else {
                                            $showOperatorAnswer =
                                                $jawabanParent->verifikator_jawaban_text !== null &&
                                                $jawabanParent->verifikator_jawaban_text !==
                                                    $jawabanParent->jawaban_text;
                                        }
                                    }
                                @endphp
                                <div class="bg-white rounded-lg p-4 border border-gray-200">
                                    <fieldset @if (!$isCanVerify || $statusVerifikasi !== 'belum_diverifikasi') disabled @endif>
                                        <div class="flex items-start gap-3 mb-3">
                                            <span
                                                class="inline-flex items-center justify-center min-w-[24px] h-6 bg-gray-100 text-gray-700 rounded text-xs font-semibold px-2">
                                                {{ $pertanyaan->urutan ?? $pertanyaanIndex + 1 }}
                                            </span>
                                            <p class="text-sm text-gray-900 flex-1">{{ $pertanyaan->pertanyaan }}</p>
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold {{ $nilaiTampil !== null ? ($nilaiTampil >= 0.8 ? 'bg-green-100 text-green-700' : ($nilaiTampil >= 0.5 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700')) : '' }}"
                                                style="{{ $nilaiTampil === null ? 'display: none;' : '' }}">
                                                @if ($nilaiTampil !== null)
                                                    ...
                                                    @php
                                                        $isPercenCapianFormat =
                                                            isset($pertanyaan) && $pertanyaan->has_sub_pertanyaan;
                                                    @endphp
                                                    Nilai:
                                                    {{ $isPercenCapianFormat ? number_format((float) $nilaiTampil * 100, 2) . '%' : number_format((float) $nilaiTampil, 2) }}
                                                @endif
                                            </span>
                                        </div>
                                        ...
                                        <div class="space-y-3">
                                            ...
                                            @elseif($pertanyaan->tipe_jawaban === 'angka')
                                                @if ($pertanyaan->has_sub_pertanyaan)
                                                    @include(
                                                        'page.kuesioner.partials.input-sub-pertanyaan',
                                                        [
                                                            'pertanyaan' => $pertanyaan,
                                        ...
                                        <div class="mt-3 pt-3 ml-9 border-t border-gray-100">
                                            ...
                                            <div>
                                                @php
                                                    $uploadedFiles = $jawabanParent
                                                        ? $jawabanParent->files ?? collect()
                                                        : collect();
                                                @endphp
                                                    ...
                                                    <ol class="list-decimal list-inside space-y-1 mt-1 text-[13px] ml-1">
                                                        @foreach ($uploadedFiles as $file)
                                                            <li class="text-gray-600">
                                                                <a href="{{ route('kuesioner.file.item.view', $file->id) }}"
                                                                    target="_blank"
                                                                    data-file-url="{{ route('kuesioner.file.item.view', $file->id) }}"
                                                                    class="js-view-file text-blue-600 hover:text-blue-800 hover:underline inline-flex items-center gap-1 align-middle">
                                                                ...
                                                                @if ($file->size)
                                                                    <span
                                                                        class="text-xs text-gray-400 ml-0.5">({{ number_format($file->size / 1024, 0) }}
                                                                        KB)</span>
                                                                @endif
                                                                @if ($file->revisi_ke)
                                                                    <span
                                                                        class="ml-1 text-[11px] font-semibold text-orange-600">Revisi
                                                                        ke-{{ $file->revisi_ke }}</span>
                                                                @endif
                                                            </li>
                                                        @endforeach
                                                    </ol>
                                                @elseif($jawabanParent && $jawabanParent->file_path)
                                                    <ul class="list-disc list-inside mt-1 text-[13px] ml-1">
                                                        <li class="text-gray-600">
                                                            <a href="{{ route('kuesioner.file.view', $jawabanParent->id) }}"
                                                                target="_blank"
                                                                data-file-url="{{ route('kuesioner.file.view', $jawabanParent->id) }}"
                                                                class="js-view-file text-blue-600 hover:text-blue-800 hover:underline inline-flex items-center gap-1 align-middle">
                                                                <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none"
                                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <div class="mt-4 pt-4 border-t border-gray-200">
                                        <fieldset @if (!$isCanVerify || $statusVerifikasi === 'terkirim') disabled @endif>
                                            <div
                                                class="p-4 rounded-xl border transition-all duration-200 {{ $statusVerifikasi === 'belum_diverifikasi' ? 'bg-yellow-50/50 border-yellow-200' : 'bg-gray-50 border-gray-200' }}">
                                                ...
                                                <div
                                                    ...
                                                    <div>
                                                        <div class="flex items-center gap-3 mb-1.5">
                                                            ...
                                                            @if ($statusVerifikasi === 'belum_diverifikasi')
                                                                <span
                                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-yellow-100 text-yellow-800 border border-yellow-200 shadow-sm animate-pulse">
                                                            ...
                                                            @elseif($statusVerifikasi === 'direvisi')
                                                                <span
                                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-orange-100 text-orange-800 border border-orange-200 shadow-sm">
                                                                    <svg class="w-3.5 h-3.5" fill="none"
                                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round"
                                                                            stroke-linejoin="round" stroke-width="2"
                                                                    ...
                                                                    Sedang Direvisi Operator
                                                                    @if ($revisiCount > 0)
                                                                        <span
                                                                            class="ml-1 text-orange-600">({{ $revisiCount }}x)</span>
                                                                    @endif
                                                                </span>
                                                            @elseif($statusVerifikasi === 'terkirim')
                                                                <span
                                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-blue-100 text-blue-800 border border-blue-200 shadow-sm">
                                                                    <svg class="w-3.5 h-3.5" fill="none"
                                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round"
                                                                            stroke-linejoin="round" stroke-width="2"
                                                            ...
                                                            @else
                                                                <span
                                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-green-100 text-green-800 border border-green-200 shadow-sm">
                                                                    <svg class="w-3.5 h-3.5" fill="none"
                                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round"
                                                                            stroke-linejoin="round" stroke-width="2"
                                                        ...
                                                        <p class="text-xs text-gray-500">
                                                            @if ($statusVerifikasi === 'direvisi')
                                                                Permintaan revisi sudah dikirim ke operator. Menunggu
                                                                operator melakukan perbaikan.
                                                                @if ($statusVerifikasi === 'direvisi' && $jawabanParent?->catatan_revisi)
                                                                    <div class="mt-1 pt-3">
                                                                        <div
                                                                            class="bg-orange-50 border border-orange-200 rounded-lg px-3 py-2">
                                                                            <p
                                                                                class="text-[11px] font-bold text-orange-800 uppercase tracking-wide mb-1">
                                                                                Pesan Revisi ke Operator</p>
                                                                            <p class="text-sm text-orange-900">
                                                                                {{ $jawabanParent->catatan_revisi }}</p>
                                                                        </div>
                                                                    </div>
                                                                @endif
                                                            @elseif($statusVerifikasi === 'terkirim')
                                                                Jawaban sudah dikirim ke Verifikator Menpan.
                                                            @elseif($menungguDicekUlang)
                                                                Operator telah merevisi jawaban ini. Mohon periksa kembali
                                                                perubahan yang dilakukan.
                                                            @elseif($statusVerifikasi === 'belum_diverifikasi')
                                                                Mohon periksa jawaban dan dokumen pendukung sebelum
                                                                menyetujui.
                                                            @else
                                                    ...
                                                    <div class="flex items-center gap-3 shrink-0">
                                                        ...
                                                        <div
                                                            class="flex items-center bg-white px-4 py-2.5 rounded-lg border border-gray-200 shadow-sm">
                                                            <input type="hidden"
                                                            ...
                                                            <label
                                                                ...
                                                                <input type="checkbox"
                                                                    ...
                                                                    class="sr-only peer">
                                                                <div
                                                                    class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-[#0E7C7B]/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#0E7C7B]">
                                                                </div>
                                                                <span
                                                                    class="ml-3 text-sm font-bold text-gray-600 peer-checked:text-[#0E7C7B] group-hover:text-gray-900 transition-colors">
                                                                    Verifikasi
                                                                </span>
                                                            </label>
                                                        </div>
                                                ...
                                                <div id="revisi-panel-{{ $pertanyaan->id }}"
                                                    class="hidden mt-4 pt-4 border-t border-orange-200">
                                                    <div class="bg-orange-50 rounded-xl p-4 border border-orange-200">
                                                        <h5
                                                            class="text-xs font-bold text-orange-800 uppercase tracking-wide mb-2 flex items-center gap-1.5">
                                                        ...
                                                        <p class="text-xs text-orange-700 mb-3">Tuliskan catatan perbaikan
                                                            yang harus dilakukan operator. Operator akan mendapat notifikasi
                                                            untuk merevisi jawaban ini.</p>
                                                        <textarea id="catatan-revisi-{{ $pertanyaan->id }}" rows="3"
                                                ...
                                                {{-- Catatan Verifikator --}}
                                                <div
                                                    class="pt-4 mt-2 border-t {{ $statusVerifikasi === 'belum_diverifikasi' ? 'border-yellow-200/60' : 'border-gray-200' }}">
                                                    <label
                                                        class="flex items-center gap-2 text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">
                                                        ...
                                                        Catatan Verifikator
                                                        <span
                                                            class="text-gray-400 font-medium normal-case">(Opsional)</span>
                                                    </label>
                                                    <textarea name="verifikasi[{{ $pertanyaan->id }}][catatan_verifikator]" rows="2"
                                            ...
                                            @if ($showOperatorAnswer)
                                                <div
                                                    class="mt-4 pt-4 border-t bg-[#FEF2F2] rounded-xl p-4 border-gray-200">
                                                    <p class="text-xs font-semibold text-gray-900 uppercase mb-2">Jawaban
                                                        Operator</p>
                                                    <div class="space-y-3">
                                                        @if ($pertanyaan->tipe_jawaban === 'ya_tidak')
                                                            @include(
                                                                'page.kuesioner.partials.input-ya-tidak',
                                                                [
                                                                    'pertanyaan' => $pertanyaan,
                                                                    'jawaban' => $jawabanParent,
                                                                    'periode' => $periode,
                                                        ...
                                                        @elseif($pertanyaan->tipe_jawaban === 'pilihan_ganda')
                                                            @php
                                                                ...
                                                                if (
                                                                    $opsiTerpilih &&
                                                                    isset($pertanyaan->penjelasan_list)
                                                                ) {
                                                                    foreach (
                                                            ...
                                                            <div class="ml-6">
                                                                <div
                                                                    ...
                                                                    <span
                                                                        class="inline-flex items-center justify-center w-5 h-5 bg-[#0E7C7B]/10 text-[#0E7C7B] rounded text-xs font-bold">
                                                                        {{ $opsiTerpilih ?? '-' }}
                                                                    </span>
                                                        ...
                                                        @elseif($pertanyaan->tipe_jawaban === 'angka')
                                                            ...
                                                            @else
                                                                @include(
                                                                    'page.kuesioner.partials.input-angka',
                                                                    [
                                                                        'pertanyaan' => $pertanyaan,
                                                                        'jawaban' => $jawabanParent,
                                                                        'periode' => $periode,
                    ...
                    @if ($isCanVerify)
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <button type="submit"
                                class="w-full inline-flex items-center justify-center gap-2 px-6 py-3 bg-[#0E7C7B] text-white rounded-lg hover:bg-[#0B6160] transition-colors font-medium">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                    ...
                    @else
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <div
                                class="w-full inline-flex items-center justify-center gap-2 px-6 py-3 bg-gray-100 text-gray-400 rounded-lg font-medium border border-gray-200 cursor-not-allowed">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    @else
                        <div
                            class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 border border-gray-200 text-gray-400 rounded-lg cursor-not-allowed">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M15 19l-7-7 7-7" />
                            </svg>
    ...
    <div id="filePreviewModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-4xl h-[80vh] bg-white rounded-xl overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 bg-gray-50">
                ...
                <button type="button" id="closeFilePreview"
                    class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg" aria-label="Tutup">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('keydown', function(e) {
                if (e.key === 'Escape' && filePreviewModal && !filePreviewModal.classList.contains(
                    'hidden')) {
                    closeFilePreviewModal();
                }
            });
            ...
            document.querySelectorAll('.js-verify-nav').forEach(function(link) {
                link.addEventListener('click', function(e) {
                    if (hasUnsavedChanges) {
                        e.preventDefault();
                        alert(
                            'Ada perubahan yang belum disimpan. Klik "Simpan Verifikasi Halaman Ini" terlebih dahulu.');
                    }
                });
            });
