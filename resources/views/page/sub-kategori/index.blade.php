...
@section('page-title', 'Data Sub Kategori')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Data Sub Kategori</h2>
            <p class="text-sm text-gray-600 mt-1">Kelola data sub kategori Zona Integritas</p>
        </div>
        <a href="{{ route('sub-kategori.create') }}"
    ...
    {{-- Filter Card --}}
    <div class="bg-white rounded-xl p-5">
        <form method="GET" action="{{ route('sub-kategori.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <div class="sm:col-span-2 lg:col-span-5 flex gap-3">
                ...
                @if(request()->hasAny(['search', 'komponen_id', 'kategori_id', 'status']))
    ...
    {{-- Data Table (Desktop) --}}
    <div class="bg-white rounded-xl overflow-hidden hidden md:block">
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Komponen</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Kategori</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Kode</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Nama Sub Kategori</th>
                        <th class="px-5 py-3.5 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">Bobot</th>
                        <th class="px-5 py-3.5 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">Urutan</th>
                        <th class="px-5 py-3.5 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">Status</th>
                        <th class="px-5 py-3.5 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-purple-100 text-purple-700">
                                {{ $subKat->kategori->komponen->kode }}
                            </span>
                        </td>
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-indigo-100 text-indigo-700">
                                {{ $subKat->kategori->kode }}
                            </span>
                            <p class="text-xs text-gray-500 mt-1">{{ Str::limit($subKat->kategori->nama, 30) }}</p>
                        </td>
                        <td class="px-5 py-4">
                        ...
                        <td class="px-5 py-4">
                            <p class="font-medium text-gray-900">{{ $subKat->nama }}</p>
                            @if($subKat->deskripsi)
                            <p class="text-sm text-gray-500 mt-0.5 line-clamp-1">{{ $subKat->deskripsi }}</p>
                            @endif
                        </td>
                        <td class="px-5 py-4 text-center">
                            <span class="text-sm font-semibold text-gray-900">{{ number_format($subKat->bobot, 2) }}%</span>
                        </td>
                        <td class="px-5 py-4 text-center">
                            <span class="text-sm text-gray-700">{{ $subKat->urutan }}</span>
                        </td>
                        <td class="px-5 py-4 text-center">
                            @if($subKat->status == 1)
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('sub-kategori.show', $subKat) }}"
                                ...
                                <a href="{{ route('sub-kategori.edit', $subKat) }}"
                                ...
                                <form action="{{ route('sub-kategori.destroy', $subKat) }}"
                                      method="POST"
                    ...
                    <tr>
                        <td colspan="8" class="px-5 py-12 text-center">
                            ...
                            <p class="text-gray-500 font-medium">Tidak ada data sub kategori</p>
                            <p class="text-sm text-gray-400 mt-1">Silakan tambah data sub kategori baru</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="grid grid-cols-1 gap-4 md:hidden">
        ...
        <div class="bg-white rounded-xl p-4 space-y-3">
            <div class="flex items-start justify-between gap-3">
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-xs font-medium bg-purple-100 text-purple-700">
                            {{ $subKat->kategori->komponen->kode }}
                        </span>
                        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-xs font-medium bg-indigo-100 text-indigo-700">
                            {{ $subKat->kategori->kode }}
                        </span>
                    ...
                    <h3 class="font-semibold text-gray-900">{{ $subKat->nama }}</h3>
                    <p class="text-xs text-gray-500 mt-0.5">{{ $subKat->kategori->nama }}</p>
                </div>
                @if($subKat->status == 1)
            ...
            @if($subKat->deskripsi)
            <p class="text-sm text-gray-600 line-clamp-2">{{ $subKat->deskripsi }}</p>
            @endif
            ...
            <div class="flex items-center gap-4 pt-2 border-t border-gray-100">
                <div class="flex items-center gap-1.5 text-sm">
                    ...
                    <span class="font-medium text-gray-900">{{ number_format($subKat->bobot, 2) }}%</span>
                </div>
                <div class="flex items-center gap-1.5 text-sm">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/>
                    </svg>
                    <span class="text-gray-600">Urutan: {{ $subKat->urutan }}</span>
                </div>
            </div>
...
@push('scripts')
<script>
// Filter kategori berdasarkan komponen yang dipilih
document.getElementById('komponen_filter')?.addEventListener('change', function() {
