...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
            <h2 class="text-2xl font-bold text-gray-900">Edit Sub Kategori</h2>
            <p class="text-sm text-gray-600 mt-1">Perbarui data sub kategori</p>
        </div>
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('sub-kategori.update', $subKategori) }}" method="POST" class="bg-white rounded-xl p-4 sm:p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="kategori_id" id="kategori_id"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none @error('kategori_id') border-red-300 @enderror">
                        <option value="">-- Pilih Kategori --</option>
                        @foreach($kategoris as $kat)
                            <option value="{{ $kat->id }}" {{ old('kategori_id', $subKategori->kategori_id) == $kat->id ? 'selected' : '' }}>
                                {{ $kat->komponen->kode }}.{{ $kat->kode }} - {{ $kat->nama }} ({{ number_format($kat->bobot, 2) }}%)
                            </option>
                        @endforeach
                    </select>
                    @error('kategori_id')
                ...
                <div>
                    ...
                    <input type="text"
                           name="kode"
                           id="kode"
                           value="{{ old('kode', $subKategori->kode) }}"
                           maxlength="10"
                           placeholder="1, 2, 3, a, b, dll"
                ...
                <div>
                    ...
                    <input type="number"
                           name="bobot"
                           id="bobot"
                           value="{{ old('bobot', $subKategori->bobot) }}"
                           min="0"
                           max="100"
                           step="0.01"
                           placeholder="0.00"
                ...
                <div>
                    ...
                    <input type="number"
                           name="urutan"
                           id="urutan"
                           value="{{ old('urutan', $subKategori->urutan) }}"
                           min="0"
                           placeholder="1"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi"
                              ...
                              class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none @error('deskripsi') border-red-300 @enderror">{{ old('deskripsi', $subKategori->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="1" {{ old('status', $subKategori->status) == '1' ? 'selected' : '' }}>Aktif</option>
                        <option value="0" {{ old('status', $subKategori->status) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
